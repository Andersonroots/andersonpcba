...
function CronogramaPage() {
  ...
  const matchBusca = (s: { titulo: string; disciplinaNome: string; topicoId?: string | null; disciplinaId: string }) => {
    ...
    return (
      ...
      (t?.f?.some((f) => normalizar(f).includes(buscaNorm)) ?? false)
    );
  };
          ...
          {resultados.length === 0 ? (
            ...
            <div className="space-y-2">
              {resultados.map(({ data, slot }) => {
                ...
                const feito = !!estado.progresso[slot.id]?.feito;
                return (
                  <div
                    key={`${data}-${slot.id}`}
                  ...
                  >
                    <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
                      ...
                      {feito && <Check className="h-4 w-4 text-primary" aria-label="Concluído" />}
                    </div>
